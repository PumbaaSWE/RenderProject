...
public:
	...
	float sens = 100;
	glm::vec3 pos{ 0 };
	glm::mat4 view = glm::mat4(1.0f); //this is the camera pos, only chage when camera changes
	glm::mat4 proj = glm::mat4(1.0f);

	Camera(const glm::vec3 pos) :pos(pos) {
		proj = glm::perspective(glm::radians(60.0f), 720.0f / 420.0f, 0.1f, 1000.0f); //this only change when fov or zNear/zFar changes
		proj[1][1] *= -1; //glm is flipped (OpenGL v Vulkan up? y neg up or down?)

		//ProjectionMatrix(glm::radians(60.0f), 720.0f / 420.0f, 0.1f, 1000.0f, proj);
	}
	...
	void Update(float dt) {
		...
		//view = glm::lookAt(pos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f)); //this is the camera pos, only chage when camera changes
